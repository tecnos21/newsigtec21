...
def run_crime_watch(exchange, symbol, ticker, funding_hr, ls_ratio):
    try:
        ...
        vol_24h  = safe_float(ticker.get("quoteVolume"))
        ...
        try:
            ...
            for symbol in pairs:
                try:
                    ...
                    print(f"[debug] {symbol} last={ticker.get('last')} quoteVolume={ticker.get('quoteVolume')} percentage={ticker.get('percentage')}")
                    price  = safe_float(ticker.get("last"))
